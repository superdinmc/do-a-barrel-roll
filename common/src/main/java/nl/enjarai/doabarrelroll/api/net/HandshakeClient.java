...
public class HandshakeClient<T> {
    ...
    public PacketByteBuf handleConfigSync(PacketByteBuf buf) {
        ...
        try {
            serverConfig = transferCodec.parse(JsonOps.INSTANCE, new JsonParser().parse(data))
                    .getOrThrow(false, error -> DoABarrelRoll.LOGGER.error("[do-a-barrel-roll] " + error));
        } catch (RuntimeException e) {
            serverConfig = null;
            DoABarrelRoll.LOGGER.error("[do-a-barrel-roll] Failed to parse config from server", e);
        }

        if (serverConfig != null) {
            updateCallback.accept(serverConfig);
            DoABarrelRoll.LOGGER.info("[do-a-barrel-roll] Received config from server");
        }
