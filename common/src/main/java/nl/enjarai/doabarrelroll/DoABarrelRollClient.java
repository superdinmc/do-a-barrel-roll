...
public class DoABarrelRollClient {
    ...
    public static void onWorldRender(MinecraftClient client, float tickDelta, MatrixStack matrix) {
        ...
        if (!isRolling()) {
            ...
            if (client.player != null) {
                var to = ElytraMath.getAssumedLeft(client.player.getYaw(0));
                left = new Vec3d(MathHelper.lerp(landingLerp, left.x, to.x), MathHelper.lerp(landingLerp, left.y, to.y), MathHelper.lerp(landingLerp, left.z, to.z));
            }
        ...
        if (client.player != null && landingLerp < 1) {
            ...
            double angle = -Math.acos(MathHelper.clamp(left.dotProduct(ElytraMath.getAssumedLeft(client.player.getYaw(0))), -1, 1)) * ElytraMath.TODEG;
    ...
    public static void changeElytraLook(double pitch, double yaw, double roll, Sensitivity sensitivity, double delta) {
        ...
        var currentRoll = ElytraMath.getRoll(player.getYaw(0), DoABarrelRollClient.left);
